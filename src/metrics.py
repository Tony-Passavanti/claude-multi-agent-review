...
def _ensure_gitignored(repo_root: Path, rel_path: str) -> None:
    ...
    try:
        # Skip a symlinked .gitignore entirely. Git does not follow a
        # symlinked .gitignore (gitignore(5) NOTES), so writing through the
        # link cannot achieve the goal — the log would stay un-ignored — and
        # would actively dirty the link's target (a possibly-tracked in-repo
        # file, or a file outside the worktree). Best-effort: in that unusual
        # setup the log simply stays untracked. (Codex P2 on #19.)
